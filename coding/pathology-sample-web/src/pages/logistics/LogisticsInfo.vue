	...
	<div>
		<Modal
	        		...
	        		<Col span="24" style="padding-bottom: 50px;">
	        			...
	        			<template v-if="curTab == 2">
	        				<template v-if="resultData.applyList">
	        					<Table @on-row-click="tableRowClick" :columns="columns" :data="resultData.applyList"></Table>
	        				</template>
	        				...
	        				<template v-else-if="resultData.sample">
		        				...
		        				<div style="padding: 10px;">
			        				...
			        				<Row><Col span="4">打印状态:</Col><Col span="20">{{resultData.sample.tagStatusDesc}}</Col></Row>
			        					...
			        					<Col span="20" style="padding: 10px 0;">
			        						<Timeline>
										        <TimelineItem style="padding: 0;" v-for="log in resultData.sample.logisticsList" :key="log.time">
										            <p style="font-size: 12px;color: #333333;">{{log.time}}-{{log.desc}}</p>
										        </TimelineItem>
										    </Timeline>
			        					</Col>
			        				</Row>
	        ...
	        <template slot="footer">
	        	...
	        	<template v-else>
	        		<Button type="primary" @click="queryId='';curTab=1;">重新查询</Button>
	        		<Button type="primary" @click="visible=false">确认</Button>
	        	</template>
	        </template>
	    </Modal>
	    
	    <Drawer :transfer="Boolean(true)" :styles="{background:'#f2f2f2'}" title="申请单详情" width="100" :closable="true" v-model="showDrawer">
	        <apply-info :siteId="siteId" ref="applyNo"></apply-info>
	    </Drawer>
...
import {applyLogisticsQuery,getApplyDetailInfo} from '@/api/sampleApi'
  	...
  	query(){
  		if(this.queryId){
  			applyLogisticsQuery({
  				...
  				if(res.data){
  					if(res.data.applyDetail){
//						this.$emit('applyNoChange','2',{
//							applyNo:this.queryId,
//							detail:res.data.applyDetail
//						});
  						this.$refs.applyNo.applyNoChange('2',{
  							applyNo:this.queryId,
  							detail:res.data.applyDetail
  						});
  	...
  	tableRowClick(index){
  		...
  		this.$refs.applyNo.applyNoChange('1',index.applyNo)
  		
//		this.$emit('applyNoChange','1',index.applyNo);
  	},
