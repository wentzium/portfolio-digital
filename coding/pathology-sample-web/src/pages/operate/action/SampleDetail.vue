	...
	<Modal
        ...
        <template slot="header">
        	<div class="modal-head-tabcard">
	        	<Tabs v-model="curTab" type="card" @on-click="tabChange">
        	...
        	<Row>
        		<Col span="10" style="padding-bottom: 50px;">
        			<template v-if="curTab == 1">
	        			...
	        			组织块数
	        			<InputNumber :max="1000" :min="1" :step="1" v-model="sampleInfo.num" style="width: 100%;"></InputNumber>
						...
						固定时间
					    <DatePicker :transfer="Boolean(true)" type="datetime"
					    	@on-open-change="sampleInfo.fixedTime?sampleInfo.fixedTime:(sampleInfo.fixedTime=moment().format('YYYY-MM-DD HH:mm:ss'))" 
					    	v-model="sampleInfo.fixedTime" show-week-numbers size="small" style="width: 100%"></DatePicker>
					   	固定液类型
					    ...
					    <Input v-model="sampleInfo.transferContainer" ><span slot="append">ml</span></Input>-->
						    手术类型
					    <Select v-model="sampleInfo.operationTypeDicId" @on-change="operationTypeChange" >
	        			...
	        			<div v-if="sampleInfo.operationTypeDesc == '活体检查'">
	        				...
	        				点包块
	        				<!--<Select v-model="memo.side" :transfer="Boolean(true)" placeholder="" style="width:40px">
						        <Option value="左">左</Option>
						        <Option value="右">右</Option>
						    </Select>侧
						    <Select v-model="memo.chunkLocation" :transfer="Boolean(true)" placeholder="" style="width:70px">
  ...
  methods: {
	...
	operationTypeChange(e){
  		this.operationTypeList.map(item=>{
  			if(item.id == e) this.sampleInfo.operationTypeDesc = item.desc;
  		})
  	},
  	...
  	show(sampleNo){
  		...
  		getSampleInfo({sampleNo}).then(res=>{
  			if(res.data) {
  				...
  				let memo =  res.data.memo?JSON.parse(res.data.memo):{side:'',chunkLocation:'',size:null}
  				if(memo.size == '') memo.size = null;
  				this.memo = memo;
  				if(this.sampleInfo.fixedTime) this.fixTimeNotNull = true;
  				else this.fixTimeNotNull = false;
  			}
  		},err=>{console.log(err)});
  	...
  	save(){
  		...
  		if(this.fixTimeNotNull && fixedTime=='') {
  			this.$Message.info("已固定样本固定时间不能为空");
  			return;
  		}
  		...
  		if(operationTypeDesc == '活体检查'){
//			desc = this.memo.side + '侧' +  this.memo.chunkLocation + '包块' + this.memo.size + 'mm';
			desc = collectionLocationDesc + this.memo.size + '点包块'
  		}
